...
export default function EmailCallToAction(props: EmailCallToActionProps){
    return (
        <div style={{ backgroundColor: props?.background_color?.hex ?? "rgb(63 98 18)" }} className='pt-3 pb-3 flex flex-col lg:flex-row h-max lg:h-52 items-center justify-center gap-5 text-white'>
            ...
            <form className="flex gap-5 w-2/5 from-white" action="/api/submit-email-call-to-action" method='POST'>
                <input placeholder='Enter email' className="w-3/4 bg-transparent border-white border-x-0 border-t-0 ring-0 focus:ring-0 text-white placeholder:text-zinc-300" type="email" required/>
