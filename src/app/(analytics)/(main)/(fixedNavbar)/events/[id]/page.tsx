...
export async function generateMetadata({ params }: PageParams): Promise<Metadata> {
    ...
    return toNextMetadata([...site.faviconMetaTags, ...event.seo, {
        tag: "meta",
        attributes: {
            property: "og:url",
...
const EventPage: React.FC<PageParams> = async ({ params }) => {
    const { event } = await getPageQuery<Props>(EventPageQuery, { variables: { eq: params.id } });
                    ...
                    <div className="divide-gray-200 text-sm font-medium leading-5 xl:col-start-1 xl:row-start-2 xl:divide-y">
                        ...
                        {event.location || event.extraLocationDetails ? (
                            <section className="flex w-full flex-col py-4">
                                ...
                                {event.location ? (
                                    <LeafMaps location={event.location} />
                                ) : null}
                            </section>
                        ) : null}
                        {event?.shopItemLink ? (
                                ...
                                <ul className="space-y-1">
                                    {event.links.map((value, i) => (
                                        <li key={i}>
                                            ...
                                            <IconLink
                                                {...value} className="w-full justify-start hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-zinc-800 dark:hover:text-zinc-50"
                                            />
