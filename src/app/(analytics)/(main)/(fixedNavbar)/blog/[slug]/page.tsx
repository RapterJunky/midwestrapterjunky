...
const Article: React.FC<PageParams> = async ({ params }) => {
    ...
    const { next, prev } = await getPageQuery<{ next: { slug: string; title: string }, prev: { slug: string; title: string } }>(GetNextArticles, {
        variables: {
            id: post.id,
    ...
    const jsonld: TechArticle = {
        ...
        url: `${process.env.VERCEL_ENV === "development" ? "http" : "https"}://${process.env.VERCEL_URL}/blog/${post.slug}`
    }
