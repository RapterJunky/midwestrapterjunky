...
export type RevaildateSettings = RevaildatePage | RevaildatePages | RevaildateTags;
...
export async function POST(request: Request) {
    try {
        ...
        switch (data.type) {
            ...
            case "tags": {
                if (!data.tags.length) throw createHttpError.BadRequest("Nothing to revailate");
        ...
        if (createHttpError.isHttpError(error)) {
            return NextResponse.json({ ...error, revaildated: false }, { status: error.statusCode });
        }
