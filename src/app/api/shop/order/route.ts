...
export async function POST(request: NextRequest) {
  try {
    ...
    if (orderResult.errors) {
      ...
      const error = orderResult.errors
        ? orderResult.errors[0] ?? { code: "UNKNOWN_ERROR" }
        : { code: "UNKNOWN_ERROR" };

      throw new ZodError([
        {
          message:
            errorResponses[error.code as keyof typeof errorResponses].message,
          code: "custom",
          fatal: true,
          path: [error.code],
        },
    ...
    if (!orderId || !netAmountDueMoney)
      throw createHttpError.InternalServerError("Failed to get order info");
    ...
    if (result?.errors || !result.payment) {
      ...
      const error = result.errors
        ? result.errors[0] ?? { code: "UNKNOWN_ERROR" }
        : { code: "UNKNOWN_ERROR" };
