...
export async function GET(request: Request) {
    try {
        ...
        const { catalog, inventoryCounts } = await getSquareQuery<CartQueryResult>(CartQuery, {
            variables: {
        ...
        const data = catalog.nodes.map(value => {
            ...
            if (value.pricingType === "VARIABLE_PRICING") {
                const rootPrice = value.item.variations.find(value => value.pricingType === "FIXED_PRICING");
                price = rootPrice?.priceMoney ?? { amount: 99999, currency: "USD" };
            }

            const inventory = inventoryCounts.nodes.find(item => item.catalog.id === value.id);
