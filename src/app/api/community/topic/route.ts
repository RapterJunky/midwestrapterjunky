...
const schema = z.object({
    ...
    notification: z.enum(["true", "false"]).transform(value => value === "true").optional(),
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const { deletedId, content, title, images, tag, notification, category, postId } = schema.parse({
        ...
        if (images?.length) {
            const newImages = await uploadFiles(formData.getAll("image") as never as File[]);
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { content, title, images, tag, category, notification } = schema.parse({
            content: formData.get("content"),
            category: formData.get("category"),
            title: formData.get("title"),
            images: formData.getAll("image"),
