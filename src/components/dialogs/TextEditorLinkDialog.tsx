...
const LinkDialog: React.FC<{ state: { url: string; open: boolean; title: string; }, close: (value: string | null) => void; }> = ({
    state,
    close,
}) => {
    return (
                            ...
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-sm bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                <form
                                    onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
                                        ...
                                        el?.setCustomValidity(url.origin.endsWith(".zip") || url.origin.endsWith(".mov") ? "Link can not be a .zip or a .mov domain." : "");
                                        el?.reportValidity();
