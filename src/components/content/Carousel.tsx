    ...
    >
      ...
      >
        {props.images.map((_, i) => (
          <button
            className="ease-[cubic-bezier(0.25,0.1,0.25,1.0)] mx-[3px] box-content h-[3px] w-[30px] flex-initial cursor-pointer border-0 border-y-[10px] border-solid border-transparent bg-white bg-clip-padding p-0 -indent-[999px] opacity-50 transition-opacity duration-[600ms] motion-reduce:transition-none"
            key={i}
            type="button"
      ...
      <div className="relative h-full w-full overflow-hidden after:clear-both after:block after:content-['']">
        {props.images.map((value, i) => (
          <div
            key={i}
            className={
              "relative float-left -mr-[100%] h-full w-full transition-transform duration-[600ms] ease-in-out motion-reduce:transition-none" +
              (i !== 0 ? " hidden" : "")
            }
            data-te-carousel-active={i === 0 ? "true" : undefined}
