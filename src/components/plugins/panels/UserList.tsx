...
export const UserList: React.FC<{
  ...
  const { data, mutate } = useSWR<Paginate<User>, Response, string>(
...
const [{ pageIndex, pageSize }, setPagination] = useState<PaginationState>({
  ...
  >(
    `/api/plugin/users?page=${pageIndex}&limit=${pageSize}`,
    (url) => AuthFetch(url).then((e) => e.json()) as Promise<Paginate<User>>, { keepPreviousData: true }
  );

  const columns = useMemo<ColumnDef<User>[]>(() => [
    {
      header: "User account infomation",
      columns: [
        {
          accessorKey: "banned",
          cell: info => {
