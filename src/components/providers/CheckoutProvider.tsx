...
const SquareWebPaymentScript =
  process.env.NEXT_PUBLIC_SQUARE_MODE === "sandbox"
...
const reducer = (state: CheckoutState, action: Actions) => {
  switch (action.event) {
    case "FINISH_BILLING": {
      return {
        ...state,
        billing: !action.payload.billingAsShipping
...
const makePayment = async (
  ...
  if (!paymentApi)
    throw new Error("Square WebPayments Api is does not exist", {
      cause: "MISSING_CHECKOUT_DATA_1",
    });
  if (!card)
    throw new Error("Card form is not vaild", {
      cause: "MISSING_CHECKOUT_DATA_2",
    });
  if (!amount || Number.isNaN(amount))
    throw new Error("Failed to get charge amount", {
      cause: "MISSING_CHECKOUT_DATA_3",
    });

  const data = checkoutState.billing ?? checkoutState.shipping;
  if (!data)
    throw new Error("Unable to verify buyer", {
      cause: "MISSING_CHECKOUT_DATA_4",
    });
  ...
  if (!order.length)
    throw new Error("Cart has less then 1 item in it.", {
      cause: "INVAILD_ORDER_2",
    });
  ...
  if (!verification?.token)
    throw new Error("Failed to obtain a verifcation token", {
  ...
  if (!request.ok) {
    const error = (
      response as {
        details: { message: string; code: string; path: string[] }[];
      }
    ).details[0];

    throw new Error(error?.message ?? "Unknown Error", {
      cause: error?.code !== "custom" ? error?.code : error.path[0],
    });
  }
...
const getCalculation = async ([url, discounts, items]: [
  string,
  Discount[],
  CartItem[],
]) => {
...
const CheckoutProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const order = useSWR<Order | undefined, Response>(
    throttledCartItems.length
      ? ["/api/shop/order-calculate", state.discounts, throttledCartItems]
      : null,
    getCalculation,
    {
  ...
  return (
    <checkoutContext.Provider
      value={{
        ...
        makePayment: () =>
          makePayment(
            order.data?.netAmountDueMoney?.amount as string | null | undefined,
            paymentApi,
            card,
            state,
          ),
    ...
    >
      ...
      <Script
        src={SquareWebPaymentScript}
        onReady={() => initPaymentsApi(setPaymentApi)}
      ></Script>
    </checkoutContext.Provider>
  );
};
