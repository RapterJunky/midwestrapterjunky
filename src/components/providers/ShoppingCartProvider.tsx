...
const ShoppingCardProvider: React.FC<React.PropsWithChildren> = ({
  children,
}) => {
  const [cartItems, setCartItems, { removeItem }] = useLocalStorageState<
    CartItem[]
  >(CART_STOARGE_KEY, {
    defaultValue: [],
  });
  ...
  >(
    cartItems.length
      ? ["/api/shop/cart", cartItems.map((value) => value.id).join(",")]
      : null,
    ([url, items]) => fetcher(`${url}?cart=${items}`),
  );
