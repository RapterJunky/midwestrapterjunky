...
const TopicActions: React.FC<{ likes: number; postId: string, ownerId: string; }> = ({ postId, ownerId, likes }) => {
    ...
    return (
        <div className="p-0.5 flex justify-end gap-1 text-neutral-600">
            <button disabled={isLoading} data-headlessui-state={data?.likedByMe ? "active" : ""} onClick={() => data?.likedByMe ? unlike("post", postId) : like("post", postId)} type="button" className="p-1 flex hover:text-black ui-active:text-red-400 disabled:text-neutral-800" title="like this post">
