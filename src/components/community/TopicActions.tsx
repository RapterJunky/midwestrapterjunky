...
const TopicActions: React.FC<{ likes: number; postId: string, ownerId: string; }> = ({ postId, ownerId, likes }) => {
    ...
    return (
        <div className="p-0.5 flex justify-end gap-1 text-neutral-600">
            <button disabled={isLoading} data-headlessui-state={data?.likedByMe ? "active" : ""} onClick={() => data?.likedByMe ? unlike("post", postId) : like("post", postId)} type="button" className="p-1 flex hover:text-black hover:bg-gray-400 hover:bg-opacity-20 rounded-sm ui-active:text-red-400 ui-active:hover:text-red-500 disabled:text-neutral-800" title="like this post">
            ...
            }} type="button" className="p-1 hover:text-black hover:bg-gray-400 hover:bg-opacity-20 rounded-sm flex items-center gap-1" title="privately flag this post for attention or send a private notification about it">
            ...
            {session.data?.user.id === ownerId ? (
                <button title="delete your post"
                    className="p-1 text-red-500 hover:text-red-700 hover:bg-gray-400 hover:bg-opacity-20 rounded-sm flex items-center gap-1"
                    onClick={async () => {
