...
const DiscountForm: React.FC<Props> = ({ checkout: [checkoutState, dispatch] }) => {
    ...
    const addDiscount = async () => {
        try {
            ...
            dispatch({ type: "addDiscount", payload: { scope: "ORDER", name: code.name, catalogObjectId: code.id } });
        } catch (error) {
            console.error(error);
        ...
        <section>
            ...
            <div className="mb-4">
                ...
                <div className="flex">
                    ...
                    <button onClick={addDiscount} type="button" className="bg-primary text-white mt-1 block shadow-sm py-2 px-3 rounded-r-sm hover:bg-primary-600">Apply</button>
                </div>
                    ...
                    <ul>
                        {checkoutState.discounts.map(discount => (
                            <li className="flex items-center" key={discount.catalogObjectId}>
                                <button onClick={() => dispatch({ type: "removeDiscount", payload: discount.catalogObjectId })} type="button" className="hover:text-red-600" aria-label="Remove discount">
