...
const ImageComment: React.FC<{
    ...
    __height: number
    hasImageRef: boolean
}> = ({ __alt, __blurDataURL, __height, __src, __width, __key, __id, hasImageRef }) => {
    ...
    const onDelete = useCallback((ev: KeyboardEvent) => {
        if (isSelected && $isNodeSelection($getSelection())) {
            ...
            if ($isImageNode(node)) {
                if (hasImageRef && __id) (editor as ExtendLexicalEditor).addDeletedImage(__id);
                node.remove();
            }
        }
        return false;
...
export class ImageNode extends DecoratorNode<React.JSX.Element> {
    ...
    static clone(node: ImageNode): ImageNode {
        return new ImageNode(node.__src, node.__alt, node.__width, node.__height, node.__blurDataURL, node.__id, node.__imageType, node.__file, node.__key);
    }
    static importJSON({ src, alt, width, height, id, blurDataURL, imageType }: SerializedImageNode): ImageNode {
        return $createImageNode({ src, alt, width, height, id, blurDataURL, imageType });
    }
    ...
    constructor(src: string, alt: string, width: number, height: number, blurDataURL?: string, id?: string, imageType?: "google" | "external", file?: File, key?: NodeKey) {
        super(key);
        ...
        this.__width = width;
        this.__blurDataURL = blurDataURL ?? "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7";
    ...
    public exportDOM(): DOMExportOutput {
        ...
        if (this.__id && this.__id.startsWith("{") && this.__id.endsWith("}") && this.imageType === "google") {
            element.setAttribute("src", this.__id);
            element.setAttribute("data-blur-url", this.__id);
        } else {
            element.setAttribute("src", this.__src);
            if (!this.__blurDataURL.startsWith("data:image/gif;")) element.setAttribute("data-blur-url", this.__blurDataURL);
        }
    ...
    public decorate(): React.JSX.Element {
        ...
        const hasImageRef = !!this.__id?.startsWith("{") && !!this.__id?.endsWith("}") && this.__imageType === "google";
...
export function $createImageNode({ src, alt, width, height, key, imageType, file, blurDataURL, id }: ImagePayload): ImageNode {
    return $applyNodeReplacement(new ImageNode(src, alt, width, height, blurDataURL, id, imageType, file, key));
}
...
function convertImageElement(domNode: Node): null | DOMConversionOutput {
    if (domNode instanceof HTMLImageElement) {
        ...
        let imageType = domNode.getAttribute("data-image-type") as "external" | "google" | null ?? "external";
        ...
        const node = $createImageNode({ alt, src, width, height, id, imageType, blurDataURL });
        return { node };
    }
    return null;
}