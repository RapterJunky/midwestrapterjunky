...
const TagInput: React.FC<Props> = ({ onBlur, onChange, value, vailidate }) => {
    ...
    const addTag = () => setInputValue((currentValue) => {
        ...
        <div>
            ...
            <div className="flex gap-2">
                <Input
                    ...
                    onKeyDown={(ev) => {
                        if (ev.key === "," || (ev.key === "Enter" && inputValue.trim().length)) {
                            ev.preventDefault();
                            addTag();
                        }
                ...
                <Button type="button" onBlur={onBlur} aria-label="Add item to tag list" size="icon">
                    <Plus />
                </Button>
            </div>
        </div>
