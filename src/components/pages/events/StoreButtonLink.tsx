...
const StoreButtonLink: React.FC<{ value: string }> = async (props) => {
  const data = await fetch(`${process.env.VERCEL_ENV === "development" ? "http" : "https"}://${process.env.VERCEL_URL}/api/products?find=${btoa(props.value)}`, {
    next: {
  ...
  return (
    <Button asChild>
      <Link href={data?.at(0)?.onlineStoreUrl ?? "/404"}>
        View Store Page
      </Link>
    </Button>
  );
