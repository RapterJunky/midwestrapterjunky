...
const ShoppingCart: React.FC = () => {
    ...
    const subtotal = useMemo(() => {
        ...
        const total = cart.reduce((acc, x, i) => {
        ...
        <>
            <div className="flex flex-1 flex-col h-full">
                {!items.length ? (
                    ...
                    <>
                        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight transition-colors first:mt-0">My Cart</h2>
                        <ScrollArea className="flex-1 px-4 sm:px-6">
                            <ul>
                                {isLoading ? items.map((_, i) => (
                                    <ShoppingCartItemSkeleton key={i} />
                                )) : cart?.map((item, i) => (
                                    <ShoppingCartItem quantity={items[i]?.quantity ?? 0} item={item} key={i} />
                                ))}
                            </ul>
                        </ScrollArea>
