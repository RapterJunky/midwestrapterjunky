...
const BillingTab: React.FC = () => {
    ...
    const onSubmit = async (value: CheckoutState) => {
        ...
        try {
            ...
            setError({ message: (error as Error).message, code: (error as Error)?.cause as string ?? "UNKOWN_ERROR" });
            setErrorDialog(true);
        }
    }
    ...
    return (
        <Form {...form}>
            <ErrorDialog open={errorDialog} setOpen={setErrorDialog} message={error?.message ?? "A Unknown error as happend."} code={error?.code ?? "UNKNOWN"} />
            <ShopLoadingDialog open={form.formState.isSubmitting} />
            <form onSubmit={form.handleSubmit(onSubmit)}>
                <div className="mb-4">
                    <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">Billing</h1>
                    <Separator />
                </div>
                <FormLabel>Card Details</FormLabel>
                <SquareForm />

                <FormField control={form.control} name="billingAsShipping" render={({ field }) => (
                ...
                {!shippingAsBilling ? (<AddressForm control={form.control} type="billing" name="Billing" comments={false} />) : null}
