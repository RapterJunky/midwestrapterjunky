...
const Calculation: React.FC = () => {
    const { state, cart, order: { data, isLoading, error } } = useCheckout();
    ...
    const subtotal = useMemo(() => {
        ...
        const total = data.lineItems?.reduce((acc, x) => {
        ...
        <section className="order-1 col-span-1 flex w-full justify-start p-6 lg:order-none">
            <div className="w-full">
                ...
                <ul>
                    {isLoading ?
                        cart.items.map((_, i) => (
                            <ShoppingCartItemSkeleton key={i} />
                        )) :
                        cart.cart?.map((value, i) => (
                            <ShoppingCartItem key={i} item={value} quantity={cart.items[i]?.quantity ?? 0} />
                        ))}
                </ul>
                <Separator />
                <ul className="pb-2">
                    ...
                    {state.discounts.length ? (
                        <li className="flex justify-between py-1">
                            ...
                            <span>
                                -
                                <RenderMoney isLoading={isLoading} error={error} code={state.currencyCode} value={data?.totalDiscountMoney?.amount} />
                            </span>
                        </li>
                    ) : null}
                    <li className="flex justify-between py-1">
                        <span>Taxes</span>
                        <span>
                            <RenderMoney isLoading={isLoading} error={error} code={state.currencyCode} value={data?.totalTaxMoney?.amount} />
                        </span>
                    </li>
                    <li className="flex justify-between py-1">
                        <span>Service Charges</span>
                        <span>
                            <RenderMoney isLoading={isLoading} error={error} code={state.currencyCode} value={data?.totalServiceChargeMoney?.amount} />
                        </span>
                    </li>
                </ul>
                <div className="border-accent-2 mb-2 flex justify-between border-t py-3">
                    ...
                    <span>
                        ...
                        <span className="font-bold tracking-wide">
                            <RenderMoney isLoading={isLoading} error={error} code={state.currencyCode} value={data?.netAmountDueMoney?.amount} />
                        </span>
                    </span>
                </div>
            </div>
