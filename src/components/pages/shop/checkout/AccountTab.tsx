    ...
    try {
      ...
      if (!response.ok) {
        ...
        form.setError("userType", {
          message: "Failed to load user account data, Please try again.",
        });
        return;
      }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormItem>
                <FormControl>
                  <div className="my-4 space-y-2">
                    <FormItem
                      className="flex items-center gap-4 border border-zinc-300 p-2 hover:shadow"
          ...
          {userType === "guest" ? (
            <>
              <FormField
                rules={{ required: "A email is required" }}
                control={form.control}
                name="email"
                render={({ field }) => (
