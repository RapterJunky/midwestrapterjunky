...
const ProductImages: React.FC<{ name: string; images: { url: string; alt: string; }[] }> = ({ name, images }) => {
    const [selected, setSelected] = useState(0);
    const [dir, setDir] = useState<"slide-in-from-left" | "slide-in-from-right">("slide-in-from-left");

    const image = images.at(selected) ?? { url: `https://api.dicebear.com/6.x/icons/png?seed=${name}`, alt: "Product Image" };
