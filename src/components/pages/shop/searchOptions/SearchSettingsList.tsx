...
const SearchSettingsList: React.FC<SearchSettingsListProps> = ({ queryKey, items, name }) => {
    return (
        <div className="relative inline-block w-full">
            ...
            <div className="absolute left-0 z-10 mb-10 mt-2 hidden w-full origin-top-left rounded-md shadow-lg lg:relative lg:block lg:shadow-none">
                <div className="shadow-xs rounded-sm bg-white lg:bg-none lg:shadow-none">
                    <div>
                        <ul>
                            <li className={`hover:bg-accent-1 hover:text-accent-8 focus:bg-accent-1 focus:text-accent-8 block text-sm leading-5 text-zinc-600 focus:outline-none lg:text-base lg:font-bold lg:tracking-wide lg:no-underline lg:hover:bg-transparent`}>
                                <Link
                            ...
                            {items.map((item, i) => (
                                <SearchOption queryKey={queryKey} name={item.name} value={item.value} key={i} />
                            ))}
                        </ul>
                    </div>
                </div>
