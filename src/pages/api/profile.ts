  ...
  try {
    ...
    switch (req.method) {
      case "GET": {
        switch (type) {
          ...
          case "square": {
            ...
            const profile = await client.customersApi.retrieveCustomer(
              user.sqaureId,
            );
      ...
      case "PUT": {
        ...
        if (!user.sqaureId)
          throw createHttpError.InternalServerError(
            "No square customer id on user.",
          );

        const profile = await client.customersApi.updateCustomer(
          user.sqaureId,
          {
            ...
            address: {
              ...
              administrativeDistrictLevel1:
                schema.address?.administrative_district_level_1,
              administrativeDistrictLevel2:
                schema.address?.administrative_district_level_2,
