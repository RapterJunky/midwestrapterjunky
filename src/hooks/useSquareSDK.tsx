...
const SquareSDKContext = createContext<SquareSDKContextProps | undefined>(undefined);
...
export const SquareSDKProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
    const [error, setError] = useState<string | undefined>();
    const [loading, setLoading] = useState<boolean>(true);
    const [payments, setPayments] = useState<Payments>();

    return (
        <SquareSDKContext.Provider value={{
            loading,
            error,
            payments,
            setLoading,
...
const useSquareSDK = (active: boolean) => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ctx.setLoading(true);
                if (!container.current || !ctx.payments) return;
                card.current = await ctx.payments.card({ style: darkModeCardStyle });
                await card.current.attach(container.current);
            } catch (e) {
                console.error(e);
        ...
        if (!card.current) init();

        return () => {
            card.current?.destroy();
            if (container.current) {
                const cssId = container.current.children[0]?.getAttribute("id")?.replace("single-card-wrapper-", "");
                if (cssId) document.head.querySelector(`style[id="sq-single-card-custom-styles-${cssId}"]`)?.remove();
            }
    ...
    return {
        ...
        async tokenize(email: string, billing: Address, amount: number, currencyCode: string): Promise<{ source: string; sourceVerification: string; }> {
            ...
            const verificationToken = await ctx.payments.verifyBuyer(result.token, {
