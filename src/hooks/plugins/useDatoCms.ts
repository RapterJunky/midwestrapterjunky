    ...
    const init = async () => {
      ...
      await connect({
        ...
        renderItemFormSidebarPanel: renderItemFormSidebarPanel
          ? (sidebarPaneId, ctx) => {
              if (
                data.page === "renderItemFormSidebarPanel" &&
                data?.id === sidebarPaneId
              )
                return;
              setData({
                page: "renderItemFormSidebarPanel",
                ctx,
                id: sidebarPaneId,
              });
            }
        ...
        renderItemFormOutlet: renderItemFormOutlet
          ? (itemFormOutletId, ctx) => {
              if (
                data.page === "renderItemFormOutlet" &&
                data?.id === itemFormOutletId
              )
                return;
              setData({
                page: "renderItemFormOutlet",
                ctx,
                id: itemFormOutletId,
              });
            }
        ...
        renderAssetSource: renderAssetSource
          ? (assetSourceId, ctx) => {
              if (
                data.page === "renderAssetSource" &&
                data?.id === assetSourceId
              )
                return;
        ...
        renderFieldExtension: renderFieldExtension
          ? (fieldExtensionId, ctx) => {
              if (
                data.page === "renderFieldExtension" &&
                data?.id === fieldExtensionId
              )
                return;
              setData({
                page: "renderFieldExtension",
                ctx,
                id: fieldExtensionId,
              });
            }
        ...
        renderManualFieldExtensionConfigScreen:
          renderManualFieldExtensionConfigScreen
            ? (fieldExtensionId, ctx) => {
                if (
                  data.page === "renderManualFieldExtensionConfigScreen" &&
                  data?.id === fieldExtensionId
                )
                  return;
                setData({
                  page: "renderManualFieldExtensionConfigScreen",
                  ctx,
                  id: fieldExtensionId,
                });
              }
