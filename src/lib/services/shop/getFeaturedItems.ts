...
const fetchShopify = async (data: StorefontsProducts): Promise<Products[]> => {
    ...
    const content = await getShopifyQuery<{ [key: `item_${number}`]: Storefront.Product }>(query, {
        args: {
...
const fetchSquare = async (data: StorefontsProducts): Promise<Products[]> => {
    ...
    return catalog.nodes.map((value, i) => {
        const price = value.variations.find(value => value.pricingType === "FIXED_PRICING");

        const amount = ((price?.priceMoney?.amount ?? 9999999) / 100).toLocaleString(undefined, {
            style: "currency",
        ...
        return {
            ...
            product: {
                title: value.title,
                onlineStoreUrl: `${process.env.VERCEL_ENV === "development" ? "http" : "https"}://${process.env.VERCEL_URL}/shop/product/${value.id}`,
...
const getFeaturedItems = cache(async (items: string[]): Promise<Storefront.Product[]> => {
    ...
    for (const [idx, item] of items.entries()) {
        const [storefront, tenant, product] = item.split("$") as EncodeProductItem;
        ...
        if (!query.has(storefront)) {
            query.set(storefront, { keys: keyGeneration(storefront, tenantId), products: [] });
        }
