...
export default async function getShopifyQuery<T>(query: string, opts: QueryOptions) {
    ...
    const { data } = await dedupedFetch(JSON.stringify({
        ...
        request: {
            ...
            headers: {
                ...
                "X-Shopify-Storefront-Access-Token": opts.args.SHOPIFY_STOREFRONT_ACCESS_TOKEN,
            },
