...
const dedupedFetch = cache(async (requestData: string) => {
    ...
    const url = process.env.VERCEL_ENV === "production" ? "https://connect.squareup.com/public/graphql" : "https://connect.squareupsandbox.com/public/graphql";

    const response = await fetch(url, request);
    const responseBody = await response.json() as { data: unknown, errors?: { code: string; message: string, path: string[] }[] };
...
export default async function getSquareQuery<T>(query: string, opts?: QueryOptions) {
