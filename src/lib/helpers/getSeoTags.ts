...
const host = `${process.env.VERCEL_ENV === "development" ? "http" : "https"}://${process.env.VERCEL_URL}`;
...
const defaultSeo = ({ title, description, slug = "/", category, robots = true }: NonNullable<Opts["seo"]>): Metadata => {
...
const getSeoTags = async ({ parent, metadata, datocms, seo }: Opts): Promise<Metadata> => {
