...
export const imageDataSchema = z.string().transform((str, ctx) => {
  try {
...
const parseForm = <T extends z.AnyZodObject>(
  ...
  method: "POST" | "PATCH",
  schema: T
): Promise<{ fields: z.infer<T>; files: Record<`image[${string}]`, File>, imagesBlocks: DastReacord[] }> => {
  const creds = JSON.parse(process.env.GOOGLE_SERVICE_KEY) as NodeJS.GoogleServiceKey;
  ...
  return new Promise((resolve, reject) => {
    ...
    form.on("field", (field, value) => {
      ...
      if (hasKey.data === "editId" && method === "PATCH" && !result.data) throw createHttpError.BadRequest("Missing editId from request.");
    });
