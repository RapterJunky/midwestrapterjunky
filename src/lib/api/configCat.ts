...
class LazyLoadConfigCat {
  ...
  private async getConfig() {
    const p = memoryCache.get(process.env.CONFIG_CAT_KEY);

    if (
      p &&
      p.timestamp + this.cacheTimeToLiveSeconds * 1000 > new Date().getTime()
    ) {
      return p;
    }
