...
export default async function validate(opt: Options): Promise<ValidationResult> {
    try {
        ...
        if (opt.validateMx ?? true) {
            ...
            if (opt.validateSMTP ?? true) await checkSMTP(opt.sender ?? "name@example.org", opt.email, mx.exchange);
        }
